    ...
    const stakeExistingDelegations = async (e: any) => {
      ...
      return (
          ...
          <div className="summary-existing-delegations-pane d-flex mt-4 justify-content-center align-items-center flex-column">
              ...
              <h5 className="mt-4"> Total Stake: <span className="font-bold">{totalStake/(Math.pow(10, selectedNetwork.decimalPoint))} {selectedNetwork.base_denom.charAt(1).toUpperCase() + selectedNetwork.base_denom.slice(2)} </span></h5>
              ...
              <h5>qTokens Received:  <span className="font-bold">{(totalStake/(Math.pow(10, selectedNetwork.decimalPoint)))/selectedNetwork?.redemption_rate}</span></h5>
              <h6 className="mt-4"> Existing Delegations: </h6>
              {selectedExistingDelegations.map((x: any) => <>
                   <h6> {x['name']} :  <span className="font-bold">{x.coins[0].amount/(Math.pow(10, selectedNetwork.decimalPoint))} {x.coins[0].denom.charAt(1).toUpperCase() + x.coins[0].denom.slice(2)} </span></h6></> )}
