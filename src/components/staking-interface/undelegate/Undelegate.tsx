...
useEffect(() => {
    setUnbondingSum(withdrawals.reduce((partialSum: any, a: any) => partialSum + a.amount[0].amount/(Math.pow(10, selectedNetwork.decimals)), 0));
    ...
    const Unbond =  async (actionID: any) => {
      setTransactionSuccessful(false);
        ...
        value: {
          ...
          "amount": "" + (unstakingAmount * Math.pow(10, selectedNetwork.decimals))
        },
...
<table className="table mt-3">
  ...
  {withdrawals.map((row: any) =>
          <>
                ...
                <tr>
      <td>{row?.amount[0].amount/(Math.pow(10, selectedNetwork.decimals))} {selectedNetwork.base_denom.slice(1).toUpperCase()} </td>
